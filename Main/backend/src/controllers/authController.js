...
export const verifyRegistrationOTP = async (req, res) => {
  try {
    ...
    if (error.code === 'P2002') {
      ...
      return res.status(409).json({
        success: false,
        message: `${field.charAt(0).toUpperCase() + field.slice(1)} already registered. Please log in or use a different value.`
      });
    }
...
export const getPublicStats = async (req, res) => {
  try {
    console.log('[getPublicStats] Fetching statistics...');
    
    // Get total users (all registered users)
    const totalUsers = await prisma.user.count();
    console.log('[getPublicStats] Total users:', totalUsers);

    // Get total labeled segments (verified labels)
    const totalLabeledSegments = await prisma.label.count({
      where: { isVerified: true }
    });
    console.log('[getPublicStats] Total labeled segments:', totalLabeledSegments);

    // Get total annotators (trained analysts)
    const totalAnnotators = await prisma.annotator.count();
    console.log('[getPublicStats] Total annotators:', totalAnnotators);

    const responseData = {
      success: true,
      data: {
        totalUsers,
        totalLabeledSegments,
        totalAnnotators
      }
    };

    console.log('[getPublicStats] Sending response:', responseData);
