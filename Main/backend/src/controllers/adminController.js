...
export const approveLabelReview = async (req, res) => {
  try {
    ...
    // FIXED #31: Use annotatorID instead of email (assignedAnnotatorID is a UUID string, not email)
    if (!resolvedFeedbackID) {
      const relatedFeedback = await prisma.feedback.findFirst({
        where: {
          status: 'IN_PROGRESS',
          assignedAnnotatorID: label.annotator.annotatorID  // ✓ FIXED: Use ID not email
        },
...
export const getAllAnnotators = async (req, res) => {
  try {
    ...
    console.log('📋 GET /api/admin/annotators - Search:', search);
