...
const SafetyMap = ({ onClose }) => {
  ...
  // Initialize map
  const initializeSafetyMap = (retryCount = 0) => {
    if (!mapRef.current || !window.google || !window.google.maps) {
      console.log('SafetyMap: Waiting for Google Maps API or map ref...');
      if (retryCount < 20) {
        initTimeoutRef.current = setTimeout(() => initializeSafetyMap(retryCount + 1), 300);
      }
      return;
    }
    ...
    if (rect.width === 0 || rect.height === 0) {
      console.log('SafetyMap: Container has 0 dimensions, retrying...');
      if (retryCount < 20) {
        initTimeoutRef.current = setTimeout(() => initializeSafetyMap(retryCount + 1), 300);
      }
      return;
    ...
    try {
      console.log('SafetyMap: Initializing map with dimensions:', rect.width, rect.height);
      ...
      setMapInitialized(true);
      console.log('SafetyMap: Map initialized successfully');
  ...
  useEffect(() => {
    if (mapLoaded && !mapInstance.current) {
      ...
      initTimeoutRef.current = setTimeout(() => initializeSafetyMap(0), 100);
    }
  ...
  useEffect(() => {
    if (mapInitialized && mapInstance.current && Object.keys(ratings).length > 0) {
  ...
  return (
    <div className="safety-map-wrapper">
      ...
      <div className="safety-map-container" style={{ minHeight: '500px', height: '500px' }}>
        {(roadsLoading || !mapInitialized) && (
          <div className="map-loading">
            <p>{!mapInitialized ? 'Initializing map...' : 'Loading road data...'}</p>
          </div>
        )}
        <div ref={mapRef} className="safety-map" style={{ width: '100%', height: '100%', minHeight: '500px' }}></div>
