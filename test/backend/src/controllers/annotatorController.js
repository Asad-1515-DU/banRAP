...
import { validateCoordinate, validateCoordinatesDifferent, sanitizeCoordinate } from '../utils/coordinateValidation.js';
import { validateLabelData, sanitizeLabelData } from '../utils/labelValidation.js';
...
export const getAnnotatorDashboard = async (req, res) => {
  try {
    ...
    console.log('🔍 DEBUG: Searching for annotator with email:', email);
    ...
    console.log('🔍 DEBUG: Annotator found?', annotator ? 'YES' : 'NO');
    
    if (!annotator) {
      console.log('🔍 DEBUG: Querying all annotators in database...');
      const allAnnotators = await prisma.annotator.findMany({
        select: { email: true, annotatorID: true }
      });
      console.log('🔍 DEBUG: All annotators in DB:', allAnnotators);
      ...
      return res.status(404).json({
        ...
        debug: {
          searchedEmail: email,
          allAnnotatorsInDB: allAnnotators
        }
      });
    }
...
export const createRoadWithSegments = async (req, res) => {
  try {
    ...
    console.log('Creating road with segments:', { roadName, segmentsCount: segments?.length, email });
    ...
    // Validate road endpoints
    const startValidation = validateCoordinate(startPoint);
    if (!startValidation.isValid) {
      return res.status(400).json({
        success: false,
        message: `Invalid start point: ${startValidation.error}`
      });
    }

    const endValidation = validateCoordinate(endPoint);
    if (!endValidation.isValid) {
      return res.status(400).json({
        success: false,
        message: `Invalid end point: ${endValidation.error}`
      });
    }

    const differentValidation = validateCoordinatesDifferent(startPoint, endPoint);
    if (!differentValidation.isValid) {
      return res.status(400).json({
        success: false,
        message: differentValidation.error
      });
    }
    ...
    // Validate each segment's coordinates and store validated results
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (!seg.startPoint || !seg.endPoint) {
        return res.status(400).json({
          success: false,
          message: `Segment ${i + 1} must have startPoint and endPoint`
        });
      }

      const segStartValidation = validateCoordinate(seg.startPoint);
      if (!segStartValidation.isValid) {
        return res.status(400).json({
          success: false,
          message: `Segment ${i + 1} start point invalid: ${segStartValidation.error}`
        });
      }

      const segEndValidation = validateCoordinate(seg.endPoint);
      if (!segEndValidation.isValid) {
        return res.status(400).json({
          success: false,
          message: `Segment ${i + 1} end point invalid: ${segEndValidation.error}`
        });
      }

      const segDifferentValidation = validateCoordinatesDifferent(seg.startPoint, seg.endPoint);
      if (!segDifferentValidation.isValid) {
        return res.status(400).json({
          success: false,
          message: `Segment ${i + 1}: ${segDifferentValidation.error}`
        });
      }

      // Store validated segment with rounded coordinates
      validatedSegments.push({
        startPoint: { lat: segStartValidation.lat, lng: segStartValidation.lng },
        endPoint: { lat: segEndValidation.lat, lng: segEndValidation.lng }
      });
    }
    ...
    if (!annotator) {
      console.error('Annotator not found for email:', email);
      return res.status(404).json({
        success: false,
        message: 'Annotator not found. Please ensure you are logged in as an annotator.'
      });
    }
    ...
    if (annotator.isSuspended) {
      return res.status(403).json({
        success: false,
        message: 'You are suspended from creating labels. Please contact an administrator.'
      });
    }

    console.log('Annotator found and active:', annotator.annotatorID);

    // Use validated and rounded coordinates
    const sanitizedStart = { lat: startValidation.lat, lng: startValidation.lng };
    const sanitizedEnd = { lat: endValidation.lat, lng: endValidation.lng };
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const road = await tx.road.create({
        data: {
          roadName,
          rStartCoord: JSON.stringify(sanitizedStart),
          rEndCoord: JSON.stringify(sanitizedEnd),
      ...
      for (let i = 0; i < validatedSegments.length; i++) {
        ...
        try {
          ...
          console.error(`Error creating segment ${i + 1}:`, segmentError);
          throw new Error(`Failed to create segment ${i + 1}: ${segmentError.message}`);
        }
      }
      ...
      return {
        road: {
          roadID: road.roadID,
          roadName: road.roadName,
          startPoint: sanitizedStart,
          endPoint: sanitizedEnd,
          isVerified: false,
    ...
    if (error.message && error.message.startsWith('Failed to create segment')) {
      return res.status(400).json({
        success: false,
    ...
    if (error.code === 'P2003') {
      return res.status(400).json({
        success: false,
        message: 'Invalid reference in road data'
      });
    }
...
export const createLabel = async (req, res) => {
  ...
  try {
    ...
    if (latitude !== undefined || longitude !== undefined) {
      ...
      const coordValidation = validateCoordinate({ lat: latitude, lng: longitude });
    ...
    // Validate label data
    const labelDataValidation = validateLabelData({
      roadsideData,
      intersectionData,
      speedData
    });

    if (!labelDataValidation.isValid) {
      return res.status(400).json({
        success: false,
        message: 'Invalid label data',
        errors: labelDataValidation.errors
      });
    }

    // Sanitize label data
    const sanitizedData = sanitizeLabelData({
      roadsideData,
      intersectionData,
      speedData
    });
    ...
    // Check if label already exists for this annotator and segment (prevent duplicates)
    ...
    if (existingLabel) {
      return res.status(409).json({
        success: false,
        message: 'You have already labeled this segment. Please create a new segment or wait for admin feedback on your existing label.'
      });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (sanitizedData.roadsideData && Object.values(sanitizedData.roadsideData).some(v => v !== null)) {
        await tx.roadside.create({
          data: {
            labelID: label.labelID,
            leftObject: sanitizedData.roadsideData.leftObject || null,
            rightObject: sanitizedData.roadsideData.rightObject || null,
            distanceObject: sanitizedData.roadsideData.distanceObject || null
          }
        });
        console.log('Roadside data created for label:', label.labelID);
      }

      // Create intersection data if provided
      if (sanitizedData.intersectionData && Object.values(sanitizedData.intersectionData).some(v => v !== null)) {
        await tx.intersection.create({
          data: {
            labelID: label.labelID,
            type: sanitizedData.intersectionData.type || null,
            quality: sanitizedData.intersectionData.quality || null,
            channelisation: sanitizedData.intersectionData.channelisation || null
          }
        });
        console.log('Intersection data created for label:', label.labelID);
      }

      // Create speed data if provided
      if (sanitizedData.speedData && Object.values(sanitizedData.speedData).some(v => v !== null)) {
        await tx.speed.create({
          data: {
            labelID: label.labelID,
            speedLimit: sanitizedData.speedData.speedLimit || null,
            management: sanitizedData.speedData.management || null
          }
        });
        console.log('Speed data created for label:', label.labelID);
      }
      ...
      const riskScore = calculateRiskScore(sanitizedData.intersectionData, sanitizedData.speedData, sanitizedData.roadsideData);

      // Create or update star rating (use upsert to handle existing ratings)
      let starRating;
      try {
        starRating = await tx.starRating.upsert({
          where: {
            segmentID_roadID: {
              segmentID,
        ...
        console.log('Star rating created/updated:', starRating.ratingID);
      } catch (ratingError) {
        console.warn('Warning: Could not create/update star rating:', ratingError.message);
        // Continue even if star rating fails - it's not critical
      }
      ...
      // Create single consolidated notification for all admins about new label
      const labelingTypesStr = labelingTypes.length > 0 ? ` [${labelingTypes.join(', ')}]` : '';
      const consolidatedMessage = `New label submitted for review on ${segment.road.roadName} - Segment ${segmentID.slice(0, 8)}${labelingTypesStr}. By: ${annotator.email}. Location: ${latitude || 'N/A'}, ${longitude || 'N/A'}`;
      ...
      for (const admin of allAdmins) {
        try {
          ...
          console.warn('Warning: Could not create notification for admin', admin.email, ':', notifError.message);
        }
      }
      ...
      try {
        ...
        console.warn('Warning: Could not create notification for annotator:', notifError.message);
      }
    ...
    if (error.code === 'P2002') {
      errorMessage = 'This label already exists. Each annotator can only submit one label per segment.';
      statusCode = 409;
    } else if (error.code === 'P2025') {
      errorMessage = 'Referenced resource not found (segment or annotator does not exist)';
      statusCode = 404;
    } else if (error.code === 'P2003') {
      errorMessage = 'Invalid reference - check if segment and annotator exist';
      statusCode = 400;
